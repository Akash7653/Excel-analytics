  ...
  return (
    <div className="p-6 max-w-6xl mx-auto mt-10 bg-gradient-to-br from-blue-50 to-purple-100 rounded-lg shadow-md">
      ...
      <div className="flex flex-col gap-4 mb-6">
        <div>
          <label className="block mb-2 font-medium">Excel File (.xlsx, .xls):</label>
          <input
            type="file"
        ...
        {isAdmin && (
          <div>
            <label className="block mb-2 font-medium">Admin Upload Password:</label>
            <input
              type="password"
        ...
        <button
          ...
          className={`btn ${loading ? 'loading' : isAdmin ? 'btn-secondary' : 'btn-primary'}`}
          disabled={loading}
        >
              ...
              <thead className="bg-indigo-200">
                <tr>
                  {Object.keys(data[0]).map((key, i) => (
                    <th key={i} className="p-3 text-left font-semibold">{key}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {data.map((row, i) => (
                  <tr key={i} className="hover:bg-indigo-50">
                    {Object.values(row).map((val, j) => (
                      <td key={j} className="p-2 border">{val?.toString()}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
